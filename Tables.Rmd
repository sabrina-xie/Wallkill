...
```{r measurements}
df <- read.csv("sturgeon_WaterColumn_data_LONG.csv")
units <- df %>% 
...
wide <- df %>% 
  select(-c(unit))
wide <- pivot_wider(wide,names_from=c(chemical_name,strata),values_from=result_value,names_sep="_")

measurements.summary <- describe(wide)
setDT(measurements.summary, keep.rownames = "param")
...
measurements.summary <- measurements.summary %>% 
  ...
  arrange(param)
measurements.summary$param <- substr(measurements.summary$param,1,nchar(measurements.summary$param)-3)

measurements.summary <- left_join(measurements.summary,units,by=c("param" = "chemical_name"))
measurements.summary <- measurements.summary[,c(1,8,9,2,3,5,4,7,6)]

measurements.summary$param <- tolower(measurements.summary$param)
...
list_of_depth <- split_across(depth_ht,c(34,97,110,225,338,438,514))
